...
export const splitScriptIntoScenesFlow = ai.defineFlow({
  ...
  const bracketSplits = script.split(/(?=\[[A-Za-z0-9 _\-]{2,}[:\]]+)/).map(s => s.trim()).filter(Boolean);
  let paragraphs = bracketSplits.length >= 2 ? bracketSplits : script.split(/\n\n+/).map(p => p.trim()).filter(Boolean);
    ...
    if (paragraphs.length > 2) {
      ...
      if (outroRegex.test(paragraphs[paragraphs.length - 1])) paragraphs = paragraphs.slice(0, -1);
    }
