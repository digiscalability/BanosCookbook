...
export default function ProcessRealPDFPage() {
  ...
  type Analysis = {
    fileInfo?: { fileName: string; fileSize: string; dataUriLength: number; isLargeFile: boolean; isImagePDF: boolean };
    textExtraction?: TextExtraction;
    recommendations?: Recommendation[];
    nextSteps?: NextStep[];
  };

  const [results, setResults] = useState<Array<{ test?: string; result?: Analysis; error?: string; timestamp?: string }>>([]);
  const [currentTest, setCurrentTest] = useState<string>('');
  const [fileInfo, setFileInfo] = useState<{fileName:string;fileSize:string;dataUriLength:number;isLargeFile:boolean;isImagePDF:boolean} | null>(null);
    ...
    try {
      ...
      reader.onload = async (e) => {
        ...
        try {
          ...
          const coercedRecommendations = (generateRecommendations(textResult, file.size) as unknown as Recommendation[]).map(r => ({
          ...
          const coercedNextSteps = (generateNextSteps(textResult, file.size) as unknown as NextStep[]).map(s => ({
            step: s.step,
            title: s.title,
  ...
  const generateRecommendations = (textResult: unknown, fileSize: number) => {
    const recommendations: Array<{ type: 'warning'|'info'|'success'; title: string; message: string; solutions: string[] }> = [];
    ...
    if ((tr?.textLength ?? 0) < 100) {
      recommendations.push({
        ...
        message: 'Your MonAsal (Bana).pdf appears to be a scanned cookbook with no extractable text.',
        solutions: [
  ...
  const generateNextSteps = (textResult: unknown, fileSize: number) => {
    const steps: Array<{ step: number; title: string; description: string; action: string; priority: 'high'|'medium'|'low' }> = [];
    const tr = textResult as { textLength?: number } | undefined;
    ...
    if (fileSize > 10 * 1024 * 1024) {
      steps.push({
        ...
        description: 'Large files may be slow to process. Consider compressing or splitting into smaller files.',
            ...
            <Card>
              ...
              <CardContent>
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  <div><strong>File Size:</strong> {fileInfo.fileSize}</div>
                  <div><strong>Data URI Length:</strong> {fileInfo.dataUriLength.toLocaleString()}</div>
                  <div><strong>Type:</strong> {fileInfo.isImagePDF ? 'Image PDF (scanned)' : 'Text PDF'}</div>
                </div>
              </CardContent>
            </Card>
          )}
                              ...
                              {result.result.recommendations.map((rec, idx) => (
                                <Alert key={idx} variant={rec.type === 'warning' ? 'destructive' : rec.type === 'success' ? 'default' : 'default'}>
                                  ...
                                  <AlertDescription>
                                    ...
                                    <div className="text-sm text-gray-600 mb-2">{rec.message}</div>
                                ...
                                <Card key={idx} className="p-3">
                                  <div className="flex items-start gap-3">
                                    ...
                                    <div>
                                      ...
                                      <div className="text-sm text-blue-600 mt-1">{step.action}</div>
                                    </div>
                                  </div>
                                </Card>
                              ))}
