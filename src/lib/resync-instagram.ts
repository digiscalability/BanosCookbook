...
export async function resyncAllInstagramPosts(): Promise<{
  processed: number;
  ...
  for (const doc of snap.docs) {
    ...
    try {
      ...
      const existingInstagramCommentIds = new Set(
        existingComments
          .filter(
            (c: unknown): c is RecipeComment =>
              typeof c === 'object' &&
              c !== null &&
              'instagramCommentId' in (c as Record<string, unknown>)
          )
          .map((c: RecipeComment) => c.instagramCommentId as string)
      );
      ...
      for (const igc of igComments) {
        ...
        const timestampSafe =
          typeof igc.timestamp === 'string'
            ? new Date(igc.timestamp).toISOString()
      ...
      const commentsCount =
        typeof insights?.commentsCount === 'number' ? insights.commentsCount : 0;
