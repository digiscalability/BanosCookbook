...
// Simple store for recipes with localStorage persistence
// In a real app, this would be replaced with a database
class RecipeStore {
  private recipes: Recipe[] = [];
  private listeners: (() => void)[] = [];
  private readonly STORAGE_KEY = 'banos-cookbook-recipes';
  ...
  addRecipe(recipeData: {
    ...
    const newRecipe: Recipe = {
      ...
      author: recipeData.author || 'Anonymous',
      imageId: recipeData.selectedImageUrl || Math.floor(Math.random() * 10 + 1).toString(), // Use selected image URL or random ID
